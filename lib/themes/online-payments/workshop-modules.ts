...
export const GETTING_STARTED_MODULE: WorkshopModule = {
  ...
  estMinutes: 5,
  isPrerequisite: true,
  intro: `Welcome to Online Payments 101. Before diving into Stripe's payment APIs, take five minutes to open your Stripe Dashboard and locate your API keys. Everything in this workshop runs through your test-mode account, so no real money moves.`,
  narrative: `You just joined NovaMart, a growing e-commerce company selling artisanal home goods online. The founders have been accepting payments through a basic hosted form, but they need a proper Stripe integration. You're the developer they hired to build it. Step one: get into the Dashboard.`,
  steps: [
    {
    ...
    },
    {
      ...
      body: `Navigate to the Developers section in the left sidebar and click API keys. You'll see two keys: a publishable key (starts with pk_test_) and a secret key (starts with sk_test_). The publishable key is used in your frontend code. The secret key is used on your server and must never be exposed to the browser.\n\nNote the "Test mode" toggle in the top-right corner of the Dashboard. It should be switched on, since everything in this workshop happens in test mode.`,
      ...
      callouts: [
        {
          ...
          text: `Test mode data is completely separate from live mode data. You can create test customers, payments, and refunds without any risk. When you're ready to go live, you switch to live-mode keys and the API calls are identical.`,
        },
      ],
    },
  ],
...
export const ONLINE_PAYMENTS_WORKSHOP_MODULES: WorkshopModule[] = [
  ...
  // ── MODULE 1 ──────────────────────────────────────────────────────────
  {
    ...
    estMinutes: 5,
    intro: `Before writing any code, let's understand the ecosystem you're building on. Online payments involve multiple parties: card networks, issuing banks, acquiring banks, and payment processors, all coordinating in real time to move money from a buyer to a seller. Stripe sits in the middle and handles the complexity so you don't have to.`,
  ...
  // ── MODULE 2 ──────────────────────────────────────────────────────────
  {
    ...
    estMinutes: 8,
    intro: `The Stripe Dashboard is your control center. It's where you monitor payments, review logs, manage API keys, and configure account settings. Before writing any integration code, you need to know your way around it. This module tours the sections you'll use most and introduces the tools that will save you time during development.`,
    narrative: `NovaMart's CTO wants a walkthrough of what Stripe provides out of the box before any custom development begins. You're going to show her the Dashboard, the Logs viewer, and the event system that powers real-time notifications.`,
    ...
    steps: [
      {
        ...
        body: `Head to your Dashboard home page. The left sidebar organises everything into sections. The ones you'll use most during this workshop are:\n\n• Payments: every charge, refund, and dispute\n• Customers: saved customer records and their payment methods\n• Developers: API keys, logs, webhooks, and events\n• Settings: account configuration, branding, and team management\n\nClick through each section to see what's there. You don't need to configure anything yet, just build a mental map.`,
      ...
      },
      {
        ...
        body: `Go to Developers > Events. Stripe generates events for everything that happens in your account: payment succeeded, payment failed, customer created, refund issued, and hundreds more. Each event has a type (like payment_intent.succeeded), a timestamp, and a data payload containing the full object.\n\nEvents are the foundation of webhooks, which you'll configure in Module 5. For now, browse the event types and notice how granular they are.`,
      ...
      },
      {
        ...
        body: `Go to Settings in your Dashboard. Take note of two areas: Business settings (where you configure your public business name, support email, and statement descriptor) and Branding (where you upload your logo and set brand colors for Stripe-hosted pages like Checkout).\n\nThe statement descriptor is especially important. It's the text that appears on your customer's credit card statement. Set it to something recognizable like "NOVAMART" so customers don't file disputes because they don't recognize the charge.`,
        dashboardLink: { label: 'Settings', url: 'https://dashboard.stripe.com/settings' },
        callouts: [
          {
            kind: 'warning' as CalloutKind,
            text: `An unclear statement descriptor is one of the top causes of "friendly fraud": disputes filed by customers who genuinely don't recognize a charge on their statement. Always set a clear, recognizable descriptor.`,
          },
        ],
      },
    ],
  ...
  // ── MODULE 3 ──────────────────────────────────────────────────────────
  {
    ...
    narrative: `NovaMart's checkout page currently has a plain HTML form. The founders want it to look professional and handle card input properly. You're going to replace that form with Stripe Elements, a drop-in component that handles all the hard parts of collecting payment information securely.`,
    ...
    steps: [
      ...
      },
      {
        ...
        callouts: [
          ...
          {
            kind: 'explanation' as CalloutKind,
            text: `The clientSecret ties the frontend Element to a specific PaymentIntent on the server. This ensures the amount, currency, and payment configuration are locked server-side and can't be tampered with from the browser.`,
          },
        ],
      },
      {
  ...
  // ── MODULE 4 ──────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: `When a customer reaches checkout, your server creates a PaymentIntent:\n\nconst paymentIntent = await stripe.paymentIntents.create({\n  amount: 2999,\n  currency: 'usd',\n  automatic_payment_methods: { enabled: true },\n  metadata: { order_id: '6735' },\n})\n\nThe amount is in the smallest currency unit, so 2999 means $29.99 USD. The currency is a three-letter ISO code. automatic_payment_methods tells Stripe to enable all payment methods you've configured in your Dashboard.\n\nThe response includes a client_secret (like pi_3abc...secret_xyz). Send this to your frontend. The client_secret authorizes the browser to confirm the payment but not to modify or capture it. That's a deliberate security boundary.`,
        callouts: [
          {
            kind: 'warning' as CalloutKind,
          ...
          {
            kind: 'explanation' as CalloutKind,
            text: `The metadata field accepts up to 50 key-value pairs of arbitrary data. Use it to attach your internal order ID, customer reference, or any business data you want to see in the Dashboard and webhook payloads. Metadata isn't visible to the customer.`,
          },
        ],
        gif: {
      ...
      },
      {
        ...
        callouts: [
          {
            kind: 'tip' as CalloutKind,
            text: `Check paymentIntent.status on your server before fulfilling an order. Never rely on the frontend redirect alone, because a network interruption could prevent the redirect while the payment still succeeds. Webhooks are the reliable way to confirm payment (covered in Module 5).`,
          },
        ],
      },
      {
        title: 'Add an idempotency key',
        body: `Network errors happen. If your server sends a PaymentIntent creation request and the connection drops before receiving a response, you don't know if the PaymentIntent was created. Without protection, retrying would create a duplicate charge.\n\nIdempotency keys solve this. Pass a unique key with every write operation:\n\nconst paymentIntent = await stripe.paymentIntents.create(\n  { amount: 2999, currency: 'usd' },\n  { idempotencyKey: 'order_6735_checkout_attempt_1' }\n)\n\nIf you retry with the same key within 24 hours, Stripe returns the original response instead of creating a new PaymentIntent. Use your order ID or a UUID as the key.`,
        callouts: [
          {
            kind: 'stripe-fact' as CalloutKind,
      ...
      },
      {
        ...
        body: `You can retrieve a PaymentIntent at any time to check its current status:\n\nconst paymentIntent = await stripe.paymentIntents.retrieve('pi_3abc...')\n\nYou can also update a PaymentIntent before it's confirmed. For example, to adjust the amount if the customer modifies their cart:\n\nconst updated = await stripe.paymentIntents.update('pi_3abc...', {\n  amount: 3499,\n  metadata: { coupon_applied: 'SUMMER10' },\n})\n\nOnce a PaymentIntent has succeeded, you can't modify it. You can only refund it. If the customer changes their mind before paying, cancel the PaymentIntent explicitly with stripe.paymentIntents.cancel().`,
        dashboardLink: { label: 'Payments', url: 'https://dashboard.stripe.com/payments' },
        callouts: [
          {
            kind: 'info' as CalloutKind,
    ...
    steps: [
      ...
      },
      {
        ...
        callouts: [
          {
            kind: 'explanation' as CalloutKind,
            text: `Your webhook handler must return a 2xx HTTP status code within 30 seconds to acknowledge receipt. If Stripe doesn't receive a 2xx, it retries the webhook up to 3 times with exponential backoff. After all retries fail, the event is marked as failed in your Dashboard.`,
          },
        ],
        gif: {
  ...
  // ── MODULE 6 ──────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        body: `Stripe categorizes payment methods by how they work:\n\n• Immediate confirmation: Cards, Apple Pay, Google Pay. The customer pays and you get an immediate success or failure.\n• Redirect-based: iDEAL, Bancontact, Sofort. The customer is redirected to their bank's website to authorize the payment, then redirected back.\n• Delayed notification: ACH debits, SEPA debits. The payment is initiated but confirmation takes 2-5 business days.\n• Buy now, pay later: Klarna, Afterpay. The customer pays in installments and you receive the full amount upfront from the provider.\n\nEach type has different implications for your order fulfillment flow. Immediate methods let you fulfill instantly. Delayed methods require you to wait for a webhook confirmation before shipping.`,
        callouts: [
          {
            kind: 'explanation' as CalloutKind,
            text: `This is why webhooks (Module 5) are essential. For redirect-based and delayed payment methods, the browser redirect isn't a reliable signal. The webhook is the only guaranteed confirmation that the payment succeeded.`,
          },
        ],
      },
      {
        title: 'Test Apple Pay and Google Pay',
        body: `Apple Pay works in Safari on macOS and iOS devices with a card in your Apple Wallet. Google Pay works in Chrome with a card saved in your Google account. In test mode, both wallets use your real wallet UI but process the payment against Stripe's test environment, so no real charge is created.\n\nLoad your checkout page in the appropriate browser. If the wallet is available, the Payment Element automatically renders an Apple Pay or Google Pay button above the card form. Click it to test the flow.\n\nIf you don't see the wallet button, check: (1) you're using HTTPS (required for wallets), (2) the domain is registered in your Dashboard under Apple Pay settings, (3) the browser has a card configured in its wallet.`,
        callouts: [
          {
            kind: 'tip' as CalloutKind,
            text: `For local development, use the Stripe CLI's built-in HTTPS proxy or a tool like ngrok to serve your checkout page over HTTPS. Apple Pay and Google Pay require a secure context and won't appear on HTTP pages.`,
          },
        ],
        gif: {
      ...
      },
      {
        ...
        body: `Link is Stripe's one-click checkout solution. When a customer pays with Link, Stripe remembers their payment details and shipping address. On their next purchase, whether on NovaMart or any other Link-enabled merchant, they can pay with a single click and a verification code.\n\nLink appears automatically in the Payment Element when enabled. The customer enters their email, and if they have a Link account, Stripe pre-fills their saved payment method and address. If they're new to Link, they're offered the option to save their details after paying.\n\nYou don't need any additional code to support Link. It's handled entirely by the Payment Element and Stripe's backend.`,
        callouts: [
          {
            kind: 'stripe-fact' as CalloutKind,
  ...
  // ── MODULE 7 ──────────────────────────────────────────────────────────
  {
    ...
    estMinutes: 10,
    intro: `3D Secure (3DS) is a protocol that adds an extra authentication step to online card payments. The cardholder is prompted to verify their identity, usually via an SMS code, a banking app notification, or biometrics. 3DS reduces fraud and shifts liability for fraudulent chargebacks from the merchant to the card issuer. This module covers how 3DS works with Stripe, when it's triggered, and how your integration handles the authentication flow.`,
    ...
    steps: [
      ...
      },
      {
        ...
        callouts: [
          {
            kind: 'info' as CalloutKind,
            text: `In the European Economic Area, SCA regulation requires 3DS on most online card payments. Stripe automatically applies 3DS when required by regulation, even if you haven't written any Radar rules. Your integration handles this seamlessly through the requires_action status.`,
          },
        ],
      },
      {
        title: 'Handle requires_action in your frontend',
        body: `When you call stripe.confirmPayment() and the payment requires 3DS, Stripe automatically renders the authentication challenge. You don't need to write any 3DS-specific frontend code. The Payment Element and confirmPayment handle it.\n\nThe flow is:\n1. Customer clicks "Pay"\n2. Your code calls stripe.confirmPayment()\n3. Stripe detects 3DS is needed\n4. The authentication popup appears automatically\n5. Customer completes or cancels authentication\n6. If successful, the payment continues to processing/succeeded\n7. If failed, confirmPayment returns an error\n\nThe key insight: your confirm flow is the same whether 3DS is triggered or not. Stripe abstracts the complexity.`,
        callouts: [
          {
            kind: 'tip' as CalloutKind,
            text: `If you're using a custom payment confirmation flow (not the Payment Element), you may need to handle the requires_action status yourself using stripe.handleNextAction(). The Payment Element flow is recommended because it handles all edge cases automatically.`,
          },
        ],
      },
      {
        ...
        callouts: [
          {
            kind: 'stripe-fact' as CalloutKind,
            text: `Stripe maintains a comprehensive list of test card numbers at stripe.com/docs/testing that simulate every scenario: declines, disputes, specific error codes, 3DS outcomes, and card brand behaviors. Bookmark this page. You'll use it constantly during development.`,
          },
        ],
        gif: {
      ...
      },
      {
        ...
        callouts: [
          {
            kind: 'warning' as CalloutKind,
            text: `Liability shift only protects against fraud disputes (reason: "fraudulent"). It doesn't protect against other dispute types like "product not received" or "product not as described." Those require different evidence regardless of 3DS status.`,
          },
        ],
      },
    ],
    ...
    steps: [
      ...
      },
      {
        ...
        callouts: [
          {
            kind: 'warning' as CalloutKind,
            text: `Don't test only the happy path. In production, roughly 5-15% of payment attempts are declined. Your customers will see decline messages regularly. Make sure they're helpful and actionable, not confusing error codes.`,
          },
          {
            kind: 'explanation' as CalloutKind,
  ...
  // ── MODULE 9 ──────────────────────────────────────────────────────────
  {
    ...
    estMinutes: 10,
    intro: `Payments don't always end at "succeeded". Customers return products, find unauthorized charges on their statements, and file disputes with their banks. Handling refunds programmatically and responding to disputes effectively are essential parts of any payment integration. This module covers the Refund API, the dispute lifecycle, and strategies for reducing chargebacks.`,
    ...
    steps: [
      {
        ...
        callouts: [
          {
            kind: 'info' as CalloutKind,
            text: `You can issue multiple partial refunds on the same payment as long as the total refunded doesn't exceed the original charge amount. Each refund creates a separate Refund object with its own ID (re_...).`,
          },
        ],
        gif: {
      ...
      },
      {
        ...
        callouts: [
          {
            kind: 'explanation' as CalloutKind,
            text: `Once you submit evidence with submit: true, it can't be modified. If you want to build evidence incrementally, omit the submit field and call update multiple times. Only submit when your evidence package is complete.`,
          },
        ],
        gif: {
  ...
  // ── MODULE 10 ─────────────────────────────────────────────────────────
  {
    ...
    estMinutes: 8,
    intro: `Your integration is built and tested. Before flipping the switch from test mode to live mode, there's a checklist of items to verify. Skipping any of these can result in failed payments, security vulnerabilities, or compliance issues that are much harder to fix once real customers are affected. This module walks through every step of going live.`,
    ...
    steps: [
      ...
      },
      {
        ...
        body: `Before you can process live payments, Stripe requires some business information. Go to Settings > Account and check for any activation requirements:\n\n• Business details: legal name, address, tax ID\n• Bank account: where Stripe deposits your payouts\n• Identity verification: personal details of the account representative\n• Website: a URL where Stripe can verify your business is real\n\nStripe reviews this information and may ask follow-up questions. Start the activation process early. It can take 1-3 business days, and you can't process live payments until it's complete.`,
      ...
      },
      {
        ...
        callouts: [
          {
            kind: 'stripe-fact' as CalloutKind,
            text: `Your integration's reliability depends on your own server uptime, webhook handling, and error recovery. Build with the assumption that any individual API call might fail, and your system remains consistent.`,
          },
          {
            kind: 'tip' as CalloutKind,
